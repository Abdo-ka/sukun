...
import 'package:core/src/network/interceptors/dio_log_interceptor.dart';
...
class DioClient with DioMixin implements Dio {
  ...
  DioClient({required this.baseUrl, List<Interceptor>? interceptors}) {
    ...
    if (interceptors != null) {
      this.interceptors.addAll([DioLogInterceptor(), ...interceptors]);
    }
  }
  ...
  String handleMessage(dynamic exceptionResponse) {
    if (exceptionResponse is Map) {
      return (exceptionResponse['message'] as String?) ??
          (exceptionResponse['error'] as String?) ??
          'An unexpected error occurred';
    }
    if (exceptionResponse is String && exceptionResponse.isNotEmpty) {
