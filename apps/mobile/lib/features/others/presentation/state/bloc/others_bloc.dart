...
//TODO: (Aya) suggest to use Cubit instead of Bloc since there is no complex event handling needed
@injectable
class OthersBloc extends Bloc<OthersEvent, OthersState> {
  ...
  FutureOr<void> _onGetCategories(
    GetOthersCategoriesEvent event,
    Emitter<OthersState> emit,
  ) async {
    emit(
      state.copyWith(status: const BlocStatus.loading()),
    );
